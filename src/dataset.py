...
def convert_img_format(src_img, tar_img):
    """
    This function converts the intensity range of the images from the range
    that's convenient for storing the images (8bit 0-255) to ranges that are
    actually helpful in the model training process.
    For the source image, we will standardize the images so that
    -std <> std = -1 <> 1
    For the target image, we will normalize the images so that
    no_lung <> lung = 0 <> 1
    """
...
def generate_dataset(dataDir, split_dataset=False, train_or_test="",
                     split_factor=0.8, random_seed=1234, verbose=True):
    """
    ...
    It then just stores everything in memory. This is very achievable since
    the entire (processed) dataset is just over 90 MBi.
    """
    ...
    if len(img_paths) != len(mask_paths):
        raise ValueError("There aren't as many masks as images. "
                         "Please remove the preprocessed folder and "
                         "rerun preprocessing.py")

    # If applicable, apply shuffling to the path lists and select
    # some for either the train or test dataset
    ...
    for subject_n in range(n_subjects):
        ...
        # We will need to transform the intensity ranges of these images for
        # proper training.
    ...
    if verbose:
        dataset_size = (sys.getsizeof(src_array) + sys.getsizeof(tar_array)) \
            / 10**9
        print(f"Completed. Dataset consists of {n_subjects} images "
              f"and is {dataset_size:.2f} GB")
